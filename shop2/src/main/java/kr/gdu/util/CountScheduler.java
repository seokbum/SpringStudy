...
public class CountScheduler {
	 ...
	 * @Scheduled(cron = "0/5 * * * * ?") 
	 * @Scheduled(fixedRate = 5000) : 밀리초 단위. 이전작업의 시작시점부터 고정간격으로 실행
	 * @Scheduled(fixedDelay = 5000) : 밀리초 단위. 이전작업의 종료시점부터 고정간격으로 실행 
	 */
	private int cnt;
...
//	@Scheduled( initialDelay = 3000, fixedRate = 5000)
	 ...
	 * 	일  : 1 ~ 31
	 * 	월  : 1 ~ 12
	 * 	요일  : 1 ~ 7(MON,TUE,WEB,THR,FRI,SAT,SUN)
	 * 
	 * 표현 방식
	 * 	* : 매번
	 * 	A/B : 주기 A ~ B 마다 실행
	 ...
	 *  0 0 0/3 * * ? : 3시간마다 한번씩 실행
	 *  0 0 12 ? * 1 : 월요일 12시에 실행 
	 *  0 0 12 ? * MON : 월요일 12시에 실행 
	 *  0 0 12 ? * 6,7 : 월요일 12시에 실행 
	 *  0 0 10 ? * SAT,SUN : 주말 10시에 실행
	 *  
	 * cron 작성 사이트 : www.cronmaker.com
	 */
	/*
	 * 6월23일 12시 20분에 시간을 알려주도록 cron 설정하기 12시20분입니다.
	 */
	@Scheduled(cron = "0 20 12 23 6 ?") 
	public void execute3() {
		System.out.println("12시 20분입니다.");
	}
	/*
	 * 1. 매일 아침 10시 30분에 환율 정보를 조회하여 db에 등록하기
	 * 2. exchange 테이블 생성하기
	 * 		create table exchange (
	 * 		eno int primary key auto_increment, -- 키값
	 * 		code varcher(10),	-- 통화 코드
	 * 		name varcher(50),	-- 통화명
	 * 		sellamt float,		-- 매도율
	 * 		buyamt float,		-- 매입율
	 * 		priamt float,		-- 기준율
